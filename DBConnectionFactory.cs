...
public static class DBConnectionFactory
{
    ...
    {
        string? connectionType = configuration
            .GetSection("ConnectionType")
            .Value?
            .ToUpper();

        if (connectionType == ConnectionTypeName.CS.ToString())
            return CastCSConnectionData(configuration, connectionType);
    ...
    private static CSConnectionData CastCSConnectionData(IConfigurationSection configuration, string connectionType)
    {
        string? connectionString = configuration.GetSection("ConnectionString").Value;

        if (string.IsNullOrWhiteSpace(connectionString))
            throw new NullReferenceException("ConnectionString not specified");

        return new CSConnectionData()
        {
    ...
    private static SQSConnectionData CastSQSConnectionData(IConfigurationSection configuration, string connectionType)
    {
